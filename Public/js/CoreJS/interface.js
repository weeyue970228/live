...
var Face = {
	de: function(str) {
		str = str.replace(/<br \/>/ig, '\n').replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/(\n)+/igm, "<br>").replace(/\[`(.*?)`\]/ig, "<img src=\"/Public/images/emojiface/$1.png\"/>");
		return str;
	},
	showFace: function() {
		var objFace = $('#showFaceInfo'),
			chatR = $("#ChatFace"),
			facePos = {
				'facel': objFace.offset().left,
				'facet': objFace.offset().top
			};
		if (chatR.is(':hidden')) {
			chatR.css({
				"left": (facePos.facel) + "px",
				"top": (facePos.facet - 182 - 11) + "px"
			}).show();
		} else {
			chatR.hide();
		}
...
var FaceGb = {
	de: function(str) {
		str = str.replace(/<br \/>/ig, '').replace(/(\n)+/igm, "").replace(/\[`(.*?)`\]/ig, "<img src=\"/Public/images/emojiface/$1.png\"/>");
		return str;
	},
	showFace: function() {

		var objFace = $('#showFaceInfoGb'),
			chatR = $("#ChatFaceGb"),
			facePos = {
				'facel': objFace.offset().left,
				'facet': objFace.offset().top
...
ObjvideoControl.prototype = {
	...
	getclientNode: function() {
		...
		if (camlive) {
			that.camlive = camlive;
		}
		playlive = that.getParam("play");
		if (playlive) {
			that.playlive = playlive;
		}
		chatdomain = that.getParam("chat");
		if (chatdomain) {
			that.chatdomain = chatdomain;
		}
	},
...
var JsInterface = {
		...
		giftloading: 0 //礼物flash装载状态 1 成功  0未成功
			,
		startEndLive:function(msg,action){//主播关闭直播
		
			$.ajax({
					type: "get",
					url: "/index.php/Show/startEndLive/action/"+action,
					async: true,
					success: function(code) {
						var data =  evalJSON(code);
		...
		showGuest: function() {
			...
			if (!objC) {
				if ($("#myVideoBox").length <= 0) {
					...
					var s = Dom.$C("script");
					s.text = "swfobject.embedSWF(\"/Public/swf/5ShowShowLivePlayer.swf?roomId=" + _show.goodNum + "&rtmpHost=" + _show.chat + "&cdn=" + _show.cdnl + "&keyframe=" + _show.zjg + "&height=" + _show.height + "&width=" + _show.width + "&fps=" + _show.fps + "&bandwidth=" + _show.zddk + "&quality=" + _show.pz + "&rtmpPort=1935&appName=5showcam\", \"JoyShowLivePlayer\", 428,316,\"10.0\", \"\", {},{wmode:\"transparent\",allowscriptaccess:\"always\"});guestflashSwf();JsInterface.guestdrag(\"myVideoBox\",\"dragguest\");";
		...
		queueStart: function()
		{
			...
			JsInterface.showFlashswf(data['data']);
			...
			setTimeout(function(){
				...
				{
					JsInterface.queueStart();
				}
				else
				{
					
					giftPlayState = 0;//准备停止队列
					//console.log(data['runTime']+"毫秒后停止队列");
					//setTimeout(function(){
						//console.log("队列已停止");
					//	giftPlayState = 0;
					//},data['runTime']);
...
//				}
			...
			{
				//等待队列结束
				var queueStart = this.queueStart;
		...
		showFlash: function(data) { //礼物展示
			...
			if (data['giftCount'] >= 1314) {
				...
				var giftIcon = data['giftIcon'];
				var effectId = -1;
				// left = parseInt(Math.random() * (100 - 48) + 48) + '%';
			...
			//-1一个 0三角形 1不显示 2六字形 3嘴形 4元宝 5心形 7 ILOVEYOU 8一生一世 9海枯石烂
				if(data['giftCount']>1){
					...
					Dom.$swfId("flashCallGift").playEffect(giftIcon, effectId, 200,aa);
					setTimeout(
						function() {
							Dom.$swfId("flashCallGift").clearDuoEffect(aa);				 
						}, 5000
					);
				}else{	
					//单个礼物赠送的 战死效果
					if ($(".my_tab").css("display") == "block") {
						$('#flashCallGiftone').css({
							"left": left,
							"top": top,
							"width": "1080px",
							"height": "800px"
					...
					var aa=parseInt(Math.random()*1000);
					Dom.$swfId("flashCallGiftone").playEffect(giftIcon, effectId, 200,aa);
					setTimeout(
						function() {
							Dom.$swfId("flashCallGiftone").clearEffect(aa);
		...
		showFlashswf: function(data) { //有swf 礼物展示
			...
			Dom.$swfId("flashCallGiftswf").playEffect(giftIcon, effectId, 200);
			//-1一个 0三角形 1不显示 2六字形 3嘴形 4元宝 5心形 7 ILOVEYOU 8一生一世 9海枯石烂
			
				if(typeof(giftShowTime[giftId])=="undefined") giftShowTime[giftId] = 10;
				setTimeout(
					function() {
						Dom.$swfId("flashCallGiftswf").clearEffect();
						Dom.$swfId("flashCallGiftswf").playEffect("", "", 200);
						
					}, giftShowTime[giftId] * 1000
				);
		...
		chatFromSocket: function(data) {
			...
			if (data.retcode) {
				...
				if (data.retcode == '000000') {
					...
					if (this.inCount == 1) { //socket link sucsess	
						this.inCount = 1;
						...
						setTimeout(function() {
							...
							if (msgArray.join("") != "") {
								$("#chat_hall").append(msgArray.join(""));
							}
							if (msgArray) {
				...
				switch (taction) {
					...
					case 41: //宠物操作
						var uid = data['uid'],
							...
							ugood = data['ugood'],
							touid = data['touid'],
							touname = decodeURIComponent(data['touname']),
							tougood = data['tougood'];
		...
		showSendMsg: function(data) { //msgType:2
			...
			try {
				if (data) {
					var tempMsg = Face.de(data["ct"]),
						time = data["timestamp"],
						ugood = data["ugood"],
						uid = data["uid"],
						uname = decodeURIComponent(data["uname"]),
						cugood = this.chatgnum(data["ugood"]),
						tougood = data["tougood"],
						touid = data["touid"],
						touname = decodeURIComponent(data["touname"]),
						ctougood = this.chatgnum(data["tougood"]),
						icon = "";

					//20141124
					if (tempMsg == 'I am alive') {
						return;
					}
					//20141124

					for (var j = 0; j < shieldWordArr.length; j++) {
						var shieldWordIndex = tempMsg.indexOf(shieldWordArr[j]);
						if (shieldWordIndex >= 0) {
							var Star = "";
							for (i = 0; i < shieldWordArr[j].length; i++) {
								Star += "*";
							}
							var reg = new RegExp(shieldWordArr[j], "g"); //创建正则RegExp对象 
							tempMsg = tempMsg.replace(reg, Star);
						}
					}
			...
			try {
				...
				switch (Saction) {
					case 33: //送红包
						...
						$("#hbrank").load('/index.php/Show/show_redbagrank/t/' + Math.random(), function(responseText, textStatus, XMLHttpRequest) {
							this;
						});
						break;
					case 3: //送礼物
				 		
						var giftIcon = obj_box.giftIcon,
						    giftLuck = obj_box.luckgift,
						    giftLuckPrize = obj_box.prize,
						    giftLuckGivePrize = obj_box.giveprize,
							giftNum = obj_box.giftCount,
							giftName = obj_box.giftName,
							giftimg = '',
							isGift = obj_box.isGift || 0,
							ugood = obj_box["userNo"],
							uid = obj_box["userId"],
							uname = decodeURIComponent(obj_box["userName"]),
							cugood = this.chatgnum(obj_box["userNo"]),
							tougood = obj_box["toUserNo"],
							touid = obj_box["toUserId"],
							touname = decodeURIComponent(obj_box['toUserName']),
							tocugood = this.chatgnum(obj_box["toUserNo"]),
							gifttop = parseInt($('#gift_history li').size()) || 0;
							
						if (giftNum) {
							var giftshow = (giftNum > 150 ? 150 : giftNum);
							for (var i = 0; i < giftshow; i++) {
								giftimg += '<img src="' + obj_box["giftPath"] + '" class="gt"/>';
							}
						}
						if (giftNum != 0) {
							var giftstr = '<p><span>' + data["timestamp"] + '</span><a href="javascript:void(0);" class=\"chatuser\" gn=' + ugood + ' id=' + uid + '>' + uname + '</a><img src="/Public/images/liang.png">' + cugood + ' 送给 <a href="javascript:void(0);" gn=' + tougood + ' class=\"chatuser\" id=' + touid + '>' + touname + '</a>' + tocugood + ':' + giftimg + giftNum + '个</p>';
						} else {
						...
						//幸运礼物提示
						if(giftLuck==1){
							var luckgiftstr="<p><span>"+ data["timestamp"] +"</span><span class='chatuser'>"+uname+"通过赠送幸运礼物获得"+giftLuckGivePrize+"秀币奖励</span></p>";
							$("#chat_hall").append(luckgiftstr);
							this.isScroll("chat_hall");
							
						}
						//礼物列表
						if (touid == _show.emceeId && giftNum != 0) { //是送个主播的礼物
							var gift_history = '<li><span>' + giftNum + '</span><img src="' + giftIcon + '" class="gt"/><em>' + giftName + '</em><a title=' + uname +  '" target="_blank">' + (gifttop + 1) + '. ' + uname + '</a></li>';
					...
					case 97://座驾
						var carid = data['ct']['carid'];
						var test1 = '<param name="movie" value="'+carList[carid]['swf']+'"><param name="quality" value="high"><param name="loop" value="false"><param name="wmode" value="transparent"><embed src="'+carList[carid]['swf']+'"  quality=high pluginspage="http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash" type="application/x-shockwave-flash" width="1100"   height="800"  wmode="transparent"></embed>';

						var giftstr = '<p>土豪 <span style="color:#FF4040" scroll="#FF4040">' + uname + '</span> 驾驶着<span style="color:#FF4040" scroll="#FF4040">'+carList[carid]['name']+'</span>入场</p><p><img src=" '+carList[carid]['icon']+'" style="float:left;height:55px; width:83px;" /></p><br /><br /><br /><br /><br />';
						document.getElementById("test1").innerHTML = test1;
						
						setTimeout(function(){
							document.getElementById("test1").innerHTML = "";
							//console.log(carList[giftName]['showTime']*1000);
						},carList[carid]['showTime']*1000);
					    break;
					case 4: //抢座
						//console.log(obj_box);
						...
						if (seatstr != "") {
							Chat.msgLen++;
							$("#chat_hall").append(seatstr);
							this.isScroll("chat_hall");
						}
						var sofa_name=$('#user_sofa .t' + obj_box["seatId"]);
						var nick4=decodeURIComponent(obj_box["userNick"]).substring(0,4);
						sofa_name.html("<img>"+nick4+"...");		
						var sofa_o = $('#user_sofa .t' + obj_box["seatId"]).find('img');

						sofa_o.attr({
							'seatnum': obj_box["seatPrice"],
							'src': obj_box["userIcon"],
							'title': decodeURIComponent(obj_box["userNick"])
						});
						//礼物列表
						var gifthisNum = parseInt($('#gift_history li').size()) || 0;
						var gift_history = '<li><span>' + obj_box["seatPrice"] + '</span><img src="/Public/images/gift/sofa.png" width="24" height="24"/><em>沙发</em><a title=' + decodeURIComponent(obj_box["userNick"]) + '" target="_blank">' + (gifthisNum + 1) + '. ' + decodeURIComponent(obj_box["userNick"]) + '</a></li>';
						$('#gift_history').append(gift_history);
						$('#get_sofa').hide();
						Chat.getUserBalance(); //用户秀币更新	
						Chat.getRankByShow(); //本场排行
					...
					case 5: //房间公告
						if (obj_box["link"] == "") {
							str = "<a href='javascript:void(0);'>"+obj_box["text"]+"</a>";
						} else {
					...
					case 9: //点歌
						Song.initVodSong();
						str = '<p><span>' + data["timestamp"] + '</span><a href="javascript:void(0);" class=\"chatuser\" gn=' + obj_box["userNo"] + ' id=' + obj_box["userId"] + '> ' + decodeURIComponent(obj_box["userName"]) + ' </a>' + this.chatgnum(obj_box["userNo"]) + ' 点歌 ' + decodeURIComponent(obj_box["songName"]) + ' <img src="/Public/images/gift/song.png" /></p>';
						break;
					case 10: //同意点歌
						Song.initVodSong();
						str = '<p><span>' + data["timestamp"] + '</span><a href="javascript:void(0);" gn=' + _show.goodNum + ' class=\"chatuser\" id=' + _show.emceeId + '> ' + _show.emceeNick + ' </a>' + this.chatgnum(_show.goodNum) + ' 同意 <a href="javascript:void(0);"  class=\"chatuser\" gn=' + obj_box["userNo"] + ' id=' + obj_box["userId"] + '> ' + decodeURIComponent(obj_box["userName"]) + ' </a>' + this.chatgnum(obj_box["userNo"]) + ' 点歌 ' + decodeURIComponent(obj_box["songName"]) + ' <img src="/Public/images/gift/song.png" /></p>';
						if (obj_box["userId"] == _show.userId) {
							Chat.getUserBalance(); //用户秀币更新	 
						}
						Chat.getRankByShow(); //本场排行
					...
					case 101: //不同意点歌
						Song.initVodSong();
						str = '<p><span>' + data["timestamp"] + '</span><a href="javascript:void(0);" gn=' + _show.goodNum + ' class=\"chatuser\" id=' + _show.emceeId + '> ' + _show.emceeNick + ' </a>' + this.chatgnum(_show.goodNum) + ' 未同意 <a href="javascript:void(0);"  class=\"chatuser\" gn=' + obj_box["userNo"] + ' id=' + obj_box["userId"] + '> ' + decodeURIComponent(obj_box["userName"]) + ' </a>' + this.chatgnum(obj_box["userNo"]) + ' 点歌 ' + decodeURIComponent(obj_box["songName"]) + ' <img src="/Public/images/gift/song.png" /></p>';
						if (obj_box["userId"] == _show.userId) {
							Chat.getUserBalance(); //用户秀币更新	 
						}
						Chat.getRankByShow(); //本场排行
					...
					case 12: //礼物、沙发、礼物周星、飞屏  大公告 
						...
						//头条 lv
						var url2 = "/index.php/Show/get_headlines";
						$.ajax({
							url: url2,
							success: function(data) {
								if(data){								
									$("#first_runway_box").html(data);
								}								
							}
						});
						//头条 lv
					...
					//console.log(obj_box);
						...
						if (userid == _show.userId && _show.userId > 0) {
							_show.sa = 1;
						}
						var muser = null;
						peq = this.getloc(this.arrPeople, userid);
						if (peq >= 0) {
							muser = this.arrPeople[peq];
						}
						meq = this.getloc(this.arrManage, userid);
						if (meq < 0) {
							this.arrManage.push(muser);
							this.reflashM(1);
							this.chatManage();
						}
						break;
					...
					case 15: //开始直播
						if (_show.deny != 4) {
							Dom.$swfId("JoyShowLivePlayer")._startEndLive(this.msg);
							var that = this;
							_show.closed = 0;
					...
					*/
						...
						var flyAraa = '<p><span>' + data["timestamp"] + '</span><a gn=' + obj_box["uno"] + ' class=\"chatuser\" id=' + obj_box["uid"] + '>' + decodeURIComponent(obj_box['uname']) + '</a>' + this.chatgnum(obj_box["uno"]) + ' 发送: <img src=' + obj_box["gift"] + ' class="gt"/></p>';
						if (flyAraa != "") {
							Chat.msgLen++;
							$("#chat_hall").append(flyAraa);
							this.isScroll("chat_hall");
						}
						var flymessage = "";
						if (obj_box['touid'] == 0) {
						...
						if (Dom.$swfId("flashFlyWord").showFlyword) {
							Dom.$swfId("flashFlyWord").showFlyword(flymessage, 48, 3);
						}
						//礼物列表
						var gift_history = '<li><span>1</span><img src="' + obj_box["gift"] + '" class="gt"/><em> 飞屏 </em><a title=' + decodeURIComponent(obj_box['uname']) +  '" target="_blank">' + (gifttop + 1) + '. ' + decodeURIComponent(obj_box['uname']) + '</a></li>';
						$('#gift_history').append(gift_history);
						Chat.getUserBalance(); //用户秀币更新	
						Chat.getRankByShow(); //本场排行
						break;
					case 24: //系统公告广播
						 ...
						 */
						var links = obj_box["links"],
							mes = obj_box["mes"],
							isbroad = obj_box["broad"];
						if (links != "") {
					...
					case 45: //小喇叭
                       ...
                       // lv
						if (this.getMsgstrs.length <= 5) {
							this.getMsgstrs.push(" <li>" + getmsgStr+'</li>');
						} else {
						...
						if (labasb.flag) {
							$('#theText ul').html(this.getMsgstrs.join(''));
							//labasb.Initialize();
							labasb.appendStr();
						...
						switch (obj_box.callMethod) {
							...
							case "giftPet":
								var giftIcon = obj_box.giftIcon,
									...
									ugood = obj_box["userNo"],
									uid = obj_box["userId"],
									uname = obj_box["userName"],
									cugood = this.chatgnum(obj_box["userNo"]),
									tougood = obj_box["toUserNo"],
									touid = obj_box["toUserId"],
									touname = obj_box['toUserName'],
									tocugood = this.chatgnum(obj_box["toUserNo"]),
									gifttop = parseInt($('#gift_history li').size()) || 0;
					...
					case 48: //魔法兔子
						...
						if (_game.rabbitstatus == 1 && _game.rabbitclosed == 1) { //try{ console.log("魔法兔子执行...");}catch(e){ }
							if (obj_box['type'] == 1) { //try{ console.log("魔法兔子开始运行游戏...");}catch(e){ }
								...
								showRabbit();
								setTimeout(function() {
									Dom.$swfId('ShellSmashRabbit').initRabbit(obj_box['mrid'], _show.emceeId, obj_box['gamelength']);
								}, 2000);
		...
		endLive: function() { //end play
			...
			for (i = 1; i <= 5; i++) {
				...
				sofa_o.attr({
					'seatnum': 0,
					'src': '/Public/images/default1.gif',
					'title': ''
				});
			}
			$('#get_sofa').hide();
		...
		getChatOnline: function(data) { //fet onlinelist
			...
			var virtualusers = udata[2]['virtualusers_str'];
			if(virtualusers!='false'){
				if(virtualusers.length>0){
					for(var i = 0; i<virtualusers.length;i++){
						
						this.person.push(JSON.stringify(virtualusers[i]));
					}
				}
			}
			...
			for (var b = 0; b < uinitnum; b++) {
				...
				var user = typeof perobj[b] == 'object'?perobj[b]:evalJSON(perobj[b]),
					utype = user["userType"];
					
				if (utype == 40) {
					this.arrManage.push(user);
					this.cntManage++;
				}
				if (utype == 20) {
		...
		chatManage: function() {
			...
			for (var key in arrManage) { //manage
				...
				if (1 == 1 || mitem["userType"] == 50) { //显身
					...
					var actBadge = mitem["actBadge"],
						sbadges = ""; //活动徽章
					if (actBadge != "") {
						sbadges = this.dealBadges(actBadge);
					}
					...
					if (mitem["vip"] != 0) { //VIP
						if (mitem["vip"] == 1) {
							strMin1 += " <span class='props vip1'></span>";
						} else if (mitem["vip"] == 2) {
							strMin1 += " <span class='props vip2'></span>";
						}
					}
					if (this.gnum(mitem["goodnum"]) != "") {
						strMin1 += " <em" + pcolor + ">" + mitem["goodnum"] + "</em>";
					}
					strMin2 += ptxt;
					...
					if (strMin1 != "") {
						managerArray.push('<p>' + strMin1 + '</p>');
					}
					if (strMin2 != "") {
						managerArray.push('<p>' + strMin2 + '</p>');
					}
		...
		chatPeople: function() {
			...
			for (var key in arrPeople) { //chatonline
				...
				if (1 == 1 || pitem["userType"] == 50) { //显身
					...
					if (pitem["userType"] == 5) { //游客僵尸
						...
						if (pitem["vip"] != 0) { //VIP
							if (pitem["vip"] == 1) {
								strOn1 += " <span class='props vip1'></span>";
							} else if (pitem["vip"] == 2) {
								strOn1 += " <span class='props vip2'></span>";
							}
						}

						if (this.gnum(pitem["goodnum"]) != "") {
							strOn1 += " <em" + pcolor + ">" + pitem["goodnum"] + "</em>";
						}
						...
						if (strOn2 != "") {
							userArray.push('<p>' + strOn2 + '</p>');
						}
						if (strOn1 != "") {
							userArray.push('<p>' + strOn1 + '</p>');
						} //123456
			...
			for (var key in arrVisistor) {
				...
				userArray.push("<li id='online_" + vitem["userid"] + "' tid='" + vitem["userid"] + "' utype='" + vitem["userType"] + "' order='" + vitem["sortnum"] + "' richlevel=0 title='" + decodeURIComponent(vitem["username"]) + "'><img class='tou_xiang' style='width:44px' src='/passport/avatar.php?uid=&size=middle'/><p><a>" + decodeURIComponent(vitem["username"]) + "</a></p></li>");
		...
		reMinorder: function() {
			var varr = this.arrVisitor.length,
				parr = this.arrPeople.length;
			if (varr > 0) {
			...
			if (data) {
				...
				if (userid < 0) { //visistor
					...
					if (this.initnum < this.minCount) {
						...
						$('#content2_2').append("<li id='online_" + udata["userid"] + "' tid='" + udata["userid"] + "' order='" + udata["sortnum"] + "' utype='" + udata["userType"] + "' title='" + decodeURIComponent(udata["username"]) + "'><img style='width:44px'  class='tou_xiang' src='/passport/avatar.php?uid=" + udata["h"] + "&size=middle'/><p><a>" + decodeURIComponent(udata["username"]) + "</a></p></li>");
		...
		openSmallf: function(uid, uname, ugood) { //this.initflash
			var that = this,
				oPos = $('.chat_online').offset(),
				tpl = "<div class=\"s_flash\"></div>",
				domf = Dom.$C("div"),
				fJson = {},
				loadf = "";
			that.inf++;
		...
		openSmallg: function(gid, uname, ugood, gcount, gname, gimg) { //initgift
			var that = this,
				oPos = $('.chat_online').offset(),
				tpl = "<div class=\"tit\"><div class=\"txt\"></div></div><div class=\"g_gif\"><img src=\"/Public/images/smallgift.gif\" /></div><div class=\"sticon\"></div>",
				domg = Dom.$C("div"),
				gJson = {};
			that.ing++;

			domg.id = "domgift" + that.ing;
			domg.className = "g_small";
			domg.innerHTML = tpl;
			document.body.appendChild(domg);
		...
		appDoAdd: function(data) {
			if (data) {
				...
				if (userid < 0) { //visistor
					this.reflashP(1);
					if (this.initnum < this.minCount) {
						$('#content2_2').append("<li id='online_" + udata["userid"] + "' tid='" + udata["userid"] + "' order='" + udata["sortnum"] + "' utype='" + udata["userType"] + "' title='" + decodeURIComponent(udata["username"]) + "'><img style='width:40px'  class='tou_xiang' src='/passport/avatar.php?uid=" + udata["h"] + "&size=middle'/><p><a>" + decodeURIComponent(udata["username"]) + "</a></p></li>");
						this.arrVisitor.push(udata);
						this.initnum++;
					}
				} else {
			...
			if (vparam) {
				switch (vparam) {
					case "live":
						...
						try {
							if (Dom.$getid("VideoStudioControl")) {
								var intStudio = VideoStudioControl.GetVersion();
							}
							_show.isHD = 1; //高清
...
var InitCache = function() {
	...
	$.getJSON("/index.php/Show/show_showData/eid/" + _show.emceeId + "/rid/" + _show.roomId + Sys.ispro + "/t/" + Math.random(), function(json) {
		if (json && json.code == 0) {
			...
			function defineVars(json) {
				...
				var udata = json.data;
				var user = udata.userInfo,
					node = udata.nodeInfo,
					show = udata.showInfo,
					egg = udata.eggInfo;
				rabbit = udata.rabbitInfo;

				_game.eggstatus = egg.status;
				_game.egginterval = egg.interval, _game.eggclosed = egg.closed;
				//_game.rabbitstatus=rabbit.status;_game.rabbitinterval=rabbit.interval,_game.rabbitclosed=rabbit.closed;
				_show.up = node.up;
				_show.down = node.down;
				_show.fmsPort = node.fmsPort;
				_show.nickname = user.nickname;

				_show.chat = node.chat == "" ? "csp" : node.chat;
			...
			function loadFlash() {
				...
				if (_show.emceeId == _show.userId) {
					initLive.id = "JoyCamLivePlayer";
				} else {
					initLive.id = "JoyShowLivePlayer";
				}
				...
				if (_show.emceeId == _show.userId) {
					_script.text = "swfobject.embedSWF(\"/Public/swf/5ShowCamLivePlayer.swf?roomId=" + _show.goodNum + "&rtmpHost=" + _show.chat + "&cdn=" + _show.cdn + "&keyframe=" + _show.zjg + "&fps=" + _show.fps + "&bandwidth=" + _show.zddk + "&width=" + _show.width + "&height=" + _show.height + "&quality=" + _show.pz + "&rtmpPort=1935&appName=5showcam\",\"JoyCamLivePlayer\",480,360,\"10.0\", \"\",{},{quality:\"high\",wmode:\"opaque\",allowscriptaccess:\"always\"});"
				} else {
					_script.text = "swfobject.embedSWF(\"/Public/swf/5ShowShowLivePlayer.swf?roomId=" + _show.goodNum + "&rtmpHost=" + _show.chat + "&cdn=" + _show.cdnl + "&keyframe=" + _show.zjg + "&fps=" + _show.fps + "&bandwidth=" + _show.zddk + "&width=" + _show.width + "&height=" + _show.height + "&quality=" + _show.pz + "&rtmpPort=1935&appName=5showcam\",\"JoyShowLivePlayer\", 480,360,\"10.0\", \"\",{},{quality:\"high\",wmode:\"opaque\",allowscriptaccess:\"always\"});"
				}
			...
			function operate() {
				if (_show.emceeId == _show.userId || _show.admin == 1) {
					$('.my_tab').show();
				}
				...
				if (_show.emceeId == _show.userId || _show.admin == 1 || _show.sa == 1) {
					$('.tdeal,.menuline').show();
				}
				if (_show.emceeId == _show.userId || _show.admin == 1) {
					$('.dmanage').show();
				}
				if (_show.emceeId == _show.userId && _show.emceeLevel >= 13) {
					$('.dblack').show();
				}
			...
			function setRoom() {
				...
				for (var i = 1; i <= 4; i++) {
					...
					var nick2=sf.nick;
                    var nick3=nick2.substring(0,4);
                    if(nick3!=''){
                    	
                    	nick3=nick3+'...';
                    }
					if (i < 5) {
						strsofa += '<li style="float:left;margin-top:0px;"><a href="javascript:void(0)" data-seat="0" class="seatlight t' + i + '"> <img src="' + sf.icon + '" title="' + sf.nick + '" seatnum="' + sf.count + '"/> '+nick3+'</a><span seatid="' + i + '"></span></li>';


					} else if (i == 4) {
						strsofa += '<li style="float:left;margin-left:320px;margin-top:' + (7 - i) * 7 + 'px;"><a href="javascript:void(0)" data-seat="0" class="seatlight t' + i + '"><img src="' + sf.icon + '" title="' + sf.nick + '" seatnum="' + sf.count + '"/><span seatid="' + i + '"></span> </a></li>';
					} else {
						strsofa += '<li style="float:left;margin-top:' + (7 - i) * 7 + 'px;"><a href="javascript:void(0)" data-seat="0" class="seatlight t' + i + '"><img src="' + sf.icon + '" title="' + sf.nick + '" seatnum="' + sf.count + '"/><span seatid="' + i + '"></span> </a></li>';
					}
			...
			function loadOtherList() {
				if (_show.userId <= 0) {
					var resentTimer = setTimeout(function() {
						Song.initVodSong();
						GiftCtrl.giftList();
						Chat.getRankByShow();
					}, 1 * 60 * 1000);
	...
	if (_show.bgimg != "") {
		$("body").css('background-image', 'url(' + _show.bgimg + ')');
	}
	var RankTimer = setInterval(function() {
		Chat.getRankByShow();
	}, 1 * 60 * 1000); //本场排行榜
	var BalanceTimer = setInterval(function() {
		Chat.getUserBalance();
	}, 5 * 60 * 1000); //用户秀币更新
	...
	var jiathis_config = {
		url: window.location,
		title: "大家好，我正在直播，邀请大家强势围观 url地址 " + window.location,
		hideMore: false
	};
}
...
var broadcast = {
	...
	submitBroadcast: function() {
		...
		if (callstate == 1) {
			...
			var smsg = "{\"msg\":[{\"_method_\":\"SubmitBroadcast\",\"action\":\"45\",\"ct\":{\"message\":\"<b class=\\\"red\\\">" + _show.nickname + "(" + _show.uroom + ")：</b><a href=\\\"javascript:void();\\\" >" + msg + "</a>\"},\"msgtype\":\"1\",\"timestamp\":\"" + WlTools.FormatNowDate() + "\",\"tougood\":\"\",\"touid\":\"0\",\"touname\":\"\",\"ugood\":\"\",\"uid\":\"0\",\"uname\":\"\"}],\"retcode\":\"000000\",\"retmsg\":\"OK\",\"token\":\""+_show.token+"\"}";
      
			Socket.emitData('broadcast',smsg);
...
myscrollbar.prototype = {
	// 广播移除  lv
  	removeStr:function(){
  		that = this;
    	var t = $("#theText> ul> li:first");
    	
		$(t).animate({
				"margin-left": "-" + $(t).width()
			}, 500, function() {
					  $(t).remove();
				  if($("#theText> ul> li").length > 0){
					  that.timeout();
				  }	                  				
			}) 
	...
	timeout:function(){
 	 ...
 	 appendStr:function(){
 			if($("#theText> ul> li").length >= 6){
				  this.removeStr();
			 }
			if($("#theText> ul >li").length == 1){
				  this.timeout();
			 }		 		 		
 	 },	
	//广播移除  lv
	...
	drag: function() {
		...
		that.sblock.mousedown(function(event) {
			...
			$(document).mousemove(function(event) {
				...
				that.thetxt.css({
					left: -Math.round((that.clen * x / that.len))
				});
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data.code == '0') {
					...
					var msg = "{\"msg\":[{\"_method_\":\"sendTietiao\",\"action\":\"47\",\"ct\":{\"add\":\"1\",\"stk\":\"" + ttid + "\",\"stke\":\"" + 100 + "\"},\"msgtype\":\"1\",\"timestamp\":\"" + WlTools.FormatNowDate()+ "\",\"tougood\":\"" + UserListCtrl.goodnum + "\",\"touid\":\"" + UserListCtrl.user_id + "\",\"touname\":\"" + UserListCtrl.nickname + "\",\"ugood\":\"" + _show.uroom + "\",\"uid\":\"" + _show.userId + "\",\"uname\":\"" + _show.uname + "\"}],\"retcode\":\"000000\",\"retmsg\":\"OK\"}";
					Socket.emitData('broadcast',msg);
...
$('#tietiaob').live("mouseenter", function(e) {
	...
	if (!isInRegion($('#tietiaob'), e.pageX, e.pageY)) {
		cc();
	}
});
...
var ccc;
var cc = function() {
	window.ccc = setTimeout("lj.checkCss()", 500);
}
