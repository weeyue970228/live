...
(function ($) {
	$.fn.KSubfield = function (options) {
		...
		function ElementControl (_thisObj, parentPage) {
			...
			if (_thisObj.divA >= 300) { // TOP
				setElementWH(parentPage.axesObjA, '300');
			} else {
				if (_thisObj.divA <= 90) {
					setElementWH(parentPage.axesObjA, '90');
				} else {
			...
			if (_thisObj.divB >= 300) { // BOTTOM
				setElementWH(parentPage.axesObjB, '300');
			} else {
				if (_thisObj.divB <= 90) {
					setElementWH(parentPage.axesObjB, '90');
				} else {
