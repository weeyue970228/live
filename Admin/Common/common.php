...
//操作日志
function userLog($userid,$opname){

	$data['admin_userid']=$userid;
	$data['action_name']=$opname;
	$data['createtime']=time();
	$data['action_ip']=get_client_ip();
	$mm=M('adminlog')->add($data);
...
function checkLoginIp(){
	...
	$ip_arr_data=M('siteconfig')->where("id=1")->getField("banip");
	$ip_arr=explode("#", $ip_arr_data);
	
	$ipRanges=array();
	foreach($ip_arr as $i_a){
		$arr=explode("-",$i_a);
		array_push($ipRanges,$arr);
	}
