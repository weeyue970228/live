...
class PassportAction extends BaseAction {
	...
	public function findBackPwd(){
		...
		if($userinfo){
			...
			$message = ereg_replace('\{\$sitename\}', $this->$sitename, $message);
		...
		$siteconifg = M("siteconfig")->find(1);
		$mail->isSMTP();                                      // Set mailer to use SMTP
		$mail->Host = $siteconifg['stmp_host'];  // Specify main and backup SMTP servers
		$mail->SMTPAuth = true;                               // Enable SMTP authentication
		$mail->Username = $siteconifg['stmp_username'];                 // SMTP username
		$mail->Password = $siteconifg['stmp_password'];                           // SMTP password
		//$mail->SMTPSecure = 'ssl';                            // Enable TLS encryption, `ssl` also accepted
		$mail->Port = 25;                                    // TCP port to connect to
		
		$mail->setFrom($siteconifg['stmp_username'], 'fmscms');
		$mail->addAddress($userinfo[0]['email'], 'Joe User');     // Add a recipient
