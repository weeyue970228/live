...
//手动设置cache缓存（memcache或redis）
function set_memcache($name,$sql,$time){
	if(S($name)){
		$info=S($name);
	}else{
		$info=M()->query($sql);
...
//==========node改====================connectionRedis===========================================
function spendVerifi($uid,$type)
{
	$redis = connectionRedis();
	$res  = $redis -> get($uid . $type);
	
	if($res){
		if($res == "1"){
			return "success";
		}else{
			$redis -> set($uid . $type,'1');
			return "success";
		}
	}else{
		$redis -> set($uid . $type,'1');
...
function PackSpendVerifi($type,$uid = "")
{
	if(empty($uid)){
		$uid = $_SESSION['uid'];
	}
	$spendVerifi = spendVerifi($uid,$type);
		
		switch ($spendVerifi) {
			case "success":
				break;
			case "fail":
				echo '{"code":"1","info":"操作频繁"}';
				exit;
				break;
			default:
				echo '{"code":"1","info":"redis 服务器异常"}';
				exit;
				break;
	}
}
function getArrayKey($array,$keys){
	$values = array();
